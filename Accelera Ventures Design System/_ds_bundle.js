/* @ds-bundle: {"format":4,"namespace":"AcceleraVenturesDesignSystem_f4ab6a","components":[],"sourceHashes":{"preview/map.js":"b59833e6efc0"},"inlinedExternals":[],"unexposedExports":[]} */

(() => {

const __ds_ns = (window.AcceleraVenturesDesignSystem_f4ab6a = window.AcceleraVenturesDesignSystem_f4ab6a || {});
...
window.AcceleraMap = function () {
  ...
  const BBOX = {
    europe: {
      type: 'MultiPoint',
      coordinates: [[-11, 35], [26, 60]]
    },
    spain: {
      type: 'MultiPoint',
      coordinates: [[-9.5, 35.8], [3.5, 44]]
    },
    iberia: {
      type: 'MultiPoint',
      coordinates: [[-10, 35.8], [4, 44]]
    }
  };
  ...
  function land(g, feats, path, fill) {
    g.append('g').selectAll('path').data(feats).join('path').attr('d', path).attr('fill', d => (typeof fill === 'function' ? fill(d) : fill) || G1).attr('stroke', W).attr('stroke-width', 0.6).attr('stroke-linejoin', 'round');
  }
  return {
    BBOX,
    INK,
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "preview/map.js", error: String((e && e.message) || e) }); }
